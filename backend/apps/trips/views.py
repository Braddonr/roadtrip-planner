...
from .serializers import (
    TripListSerializer, TripDetailSerializer, TripCreateSerializer, TripUpdateSerializer,
    StopSerializer, StopCreateSerializer, TripShareSerializer, StopReorderSerializer
)
...
class StopListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        user = self.request.user
        if not (trip.user == user or trip.shares.filter(shared_with=user, is_active=True).exists()):
...
def reorder_stops(request, trip_id):
    ...
    # Check permissions
    user = request.user
    if not (trip.user == user or trip.shares.filter(shared_with=user, is_active=True, permission_level__in=['edit', 'admin']).exists()):
