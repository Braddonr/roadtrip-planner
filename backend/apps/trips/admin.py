...
class StopInline(admin.TabularInline):
    ...
    model = Stop
    extra = 0
    fields = ('name', 'address', 'latitude', 'longitude', 'stop_type', 'order', 'arrival_time', 'departure_time')
    ordering = ['order']
...
class TripAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'user', 'route_type', 'stops_count', 'total_distance', 'total_time', 'is_public', 'created_at')
    ...
    readonly_fields = ('total_distance', 'total_time', 'estimated_fuel_cost', 'created_at', 'updated_at')
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('user').prefetch_related('stops')
...
class StopAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'trip', 'stop_type', 'order', 'latitude', 'longitude', 'created_at')
    ...
    readonly_fields = ('travel_time_to_next', 'travel_distance_to_next', 'created_at', 'updated_at')
...
class TripShareAdmin(admin.ModelAdmin):
    ...
    list_display = ('trip', 'shared_with', 'shared_by', 'permission_level', 'is_active', 'shared_at')
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('trip', 'shared_with', 'shared_by')