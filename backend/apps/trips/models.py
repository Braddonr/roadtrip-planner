...
class Trip(models.Model):
    ...
    estimated_fuel_cost = models.FloatField(default=0, help_text="Estimated fuel cost in USD")
    ...
    is_public = models.BooleanField(default=False, help_text="Whether this trip is publicly visible")
    ...
    def calculate_statistics(self):
        ...
        if stops.count() < 2:
            ...
            total_distance = sum(
                ...
                for stop in stops if stop.travel_distance_to_next
            )
            total_time = sum(
                stop.travel_time_to_next or 0 
                for stop in stops if stop.travel_time_to_next
            )
            
            self.total_distance = total_distance
            self.total_time = total_time
            self.estimated_fuel_cost = (total_distance / self.fuel_efficiency) * self.fuel_price_per_gallon
        
        self.save(update_fields=['total_distance', 'total_time', 'estimated_fuel_cost', 'updated_at'])
...
class Stop(models.Model):
    ...
    place_id = models.CharField(max_length=200, blank=True, help_text="External API place ID")
...
class TripShare(models.Model):
    ...
    shared_with = models.ForeignKey(User, on_delete=models.CASCADE, related_name='received_trip_shares')
    shared_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='sent_trip_shares')
